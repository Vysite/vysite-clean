...
function exportActionsPDF(actions: Action[], logoUrl?: string) {
  const today = new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
  const rows = actions.map(a => {
    const overdueBadge = a.overdue
      ? ' <span style="display:inline-block;background:#fee2e2;color:#dc2626;font-size:9px;font-weight:700;padding:1px 6px;border-radius:9999px;margin-left:4px">OVERDUE</span>'
      : '';
  ...
  const logoHtml = logoUrl
    ? `<img style="height:36px;max-width:160px;object-fit:contain;display:block;margin-bottom:4px" src="${logoUrl}" alt="Logo">`
    : `<div style="font-size:22px;font-weight:900;color:#f97316;letter-spacing:2px">VYSITE</div>`;
  const styles = `
  ...
  const body = `
    <div class="header">
      <div>
        ${logoHtml}
        <div class="dateline">Actions Report &middot; Generated ${today} &middot; ${actions.length} action${actions.length !== 1 ? 's' : ''}</div>
      </div>
    </div>
    <table>
    ...
    <div style="margin-top:32px;border-top:1px solid #e2e8f0;padding-top:10px;font-size:10px;color:#94a3b8;text-align:center">Generated by VYSITE &middot; ${today}</div>
  `;
  openPrintTab(buildPrintDocument('Actions Report — VYSITE', styles, body));
}

function exportSingleActionPDF(action: Action, logoUrl?: string) {
  const today = new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
  const logoHtml = logoUrl
    ? `<img style="height:36px;max-width:160px;object-fit:contain;display:block;margin-bottom:4px" src="${logoUrl}" alt="Logo">`
    : `<div style="font-size:22px;font-weight:900;color:#f97316;letter-spacing:2px">VYSITE</div>`;
  const priorityColor = action.priority === 'High' ? '#dc2626' : action.priority === 'Medium' ? '#d97706' : '#64748b';
  const dueDateColor  = action.overdue ? '#dc2626' : '#1e293b';
  const commentRows = (action.comments ?? []).map(c => `
    <tr>
      <td style="font-size:10px;color:#334155;font-weight:600;white-space:nowrap">${c.user}</td>
      <td style="font-size:10px;color:#64748b;white-space:nowrap">${new Date(c.datetime).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</td>
      <td style="font-size:11px;color:#1e293b">${c.text.replace(/</g, '&lt;')}</td>
    </tr>`).join('');
  const styles = `
    .header{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #f97316;padding-bottom:14px;margin-bottom:22px}
    .meta-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:20px}
    .meta-card{background:#f8fafc;border-radius:6px;padding:10px 14px;border:1px solid #e2e8f0}
    .meta-label{font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#94a3b8;margin-bottom:4px}
    .meta-value{font-size:13px;font-weight:600;color:#1e293b}
    .section-title{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#94a3b8;margin:18px 0 8px}
    .description{font-size:12px;color:#334155;line-height:1.6;background:#f8fafc;border-radius:6px;padding:12px 14px;border:1px solid #e2e8f0}
    table{width:100%;border-collapse:collapse;font-size:11px}
    th{background:#f1f5f9;color:#334155;font-weight:700;text-transform:uppercase;letter-spacing:.05em;padding:7px 10px;text-align:left;border-bottom:2px solid #e2e8f0}
    td{padding:7px 10px;color:#1e293b;border-bottom:1px solid #f1f5f9;vertical-align:top}
    .status-badge{display:inline-block;padding:2px 10px;border-radius:9999px;font-size:10px;font-weight:700}
  `;
  const body = `
    <div class="header">
      <div>
        ${logoHtml}
        <div style="font-size:11px;color:#666;margin-top:3px">Action &middot; Generated ${today}</div>
      </div>
      <div style="text-align:right">
        <div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#94a3b8">Ref</div>
        <div style="font-size:13px;font-family:monospace;color:#f97316;font-weight:700">#${action.id.toUpperCase()}</div>
      </div>
    </div>
    <div style="font-size:18px;font-weight:800;color:#0f172a;margin-bottom:6px">${action.title.replace(/</g, '&lt;')}</div>
    <div style="font-size:12px;color:#64748b;margin-bottom:18px">${action.projectName}</div>
    <div class="meta-grid">
      <div class="meta-card"><div class="meta-label">Status</div><div class="meta-value">${action.status}${action.overdue ? ' <span style="font-size:9px;color:#dc2626;font-weight:700"> — OVERDUE</span>' : ''}</div></div>
      <div class="meta-card"><div class="meta-label">Priority</div><div class="meta-value" style="color:${priorityColor}">${action.priority}</div></div>
      <div class="meta-card"><div class="meta-label">Owner</div><div class="meta-value">${action.owner}</div></div>
      <div class="meta-card"><div class="meta-label">Due Date</div><div class="meta-value" style="color:${dueDateColor}">${new Date(action.dueDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</div></div>
      <div class="meta-card"><div class="meta-label">Created By</div><div class="meta-value">${action.createdBy}</div></div>
      <div class="meta-card"><div class="meta-label">Created</div><div class="meta-value">${new Date(action.createdDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</div></div>
    </div>
    ${action.description ? `<div class="section-title">Description</div><div class="description">${action.description.replace(/</g, '&lt;')}</div>` : ''}
    ${commentRows ? `<div class="section-title">Comments (${(action.comments ?? []).length})</div>
      <table><thead><tr><th>Author</th><th>Date</th><th>Comment</th></tr></thead><tbody>${commentRows}</tbody></table>` : ''}
    <div style="margin-top:32px;border-top:1px solid #e2e8f0;padding-top:10px;font-size:10px;color:#94a3b8;text-align:center">Generated by VYSITE &middot; ${today}</div>
  `;
  openPrintTab(buildPrintDocument(`Action — ${action.title}`, styles, body));
}
    ...
    <div className="p-4 lg:p-6">
      ...
      {selectedIds.size > 0 && (
        <div className="flex flex-wrap items-center gap-3 mb-4 bg-[#1a2236] border border-[#1e2d4a] rounded-xl px-4 py-3">
          ...
          <button onClick={() => exportActionsPDF(selectedActions, store.settings?.logo_data_url)} className="flex items-center gap-2 px-4 py-2 bg-[#f97316] text-white rounded-lg text-sm font-semibold hover:bg-orange-600 transition-colors">
                    ...
                    <div className="flex items-start justify-between gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 shrink-0" onClick={e => e.stopPropagation()}>
                        ...
                        <RowActionsMenu actions={[
                          ...
                          ...(canExport ? [{ label: 'Export PDF', icon: Printer, onClick: () => exportSingleActionPDF(action, store.settings?.logo_data_url) }] : []),
