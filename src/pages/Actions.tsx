...
function buildActionTicketHTML(action: Action, logoUrl: string | undefined, today: string): string {
...
function exportActionsPDF(actions: Action[], logoUrl?: string) {
  ...
  const pages = actions.map((a, i) =>
    `<div${i > 0 ? ' class="page-break"' : ''}>${buildActionTicketHTML(a, logoUrl, today)}</div>`
  ).join('');
  openPrintTab(buildPrintDocument(
    actions.length === 1 ? `Action — ${actions[0].title}` : `Actions (${actions.length}) — VYSITE`,
    styles,
    pages
  ));
}
...
function exportSingleActionPDF(action: Action, logoUrl?: string) {
  ...
  const styles = `
    ...
    .status-badge{display:inline-block;padding:2px 10px;border-radius:9999px;font-size:10px;font-weight:700}
  `;
  openPrintTab(buildPrintDocument(`Action — ${action.title}`, styles, buildActionTicketHTML(action, logoUrl, today)));
}
