...
export default function Dashboard({ onNavigate, onNavigateProject }: DashboardProps) {
  ...
  const actionList = visibleProjectIds
    ...
    : store.actions;
  const snags = useMemo(() => {
    const reportIds = new Set(store.snaggingReports.map(r => r.id));
    const scoped = (store.snags as (typeof store.snags[number] & { reportId?: string })[])
      .filter(s => s.reportId && reportIds.has(s.reportId));
    return visibleProjectIds
      ? scoped.filter(s => visibleProjectIds.includes(s.projectId))
      : scoped;
  }, [store.snags, store.snaggingReports, visibleProjectIds]);
