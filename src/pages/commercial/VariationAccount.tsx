...
function VariationDrawer({
  ...
  // Pre-fetch data_urls for all attachments in the background.
  // This runs whenever the attachment list changes (drawer opens / attachment added).
  // By the time the user clicks Export, the cache is populated and the handler is synchronous.
  useEffect(() => {
    if (!attachments.length) { setAttDataCache({}); return; }
    let cancelled = false;
    Promise.all(
  ...
  // Synchronous export — same pattern as every other VYSITE PDF (openPrintTab called
  // directly with ready HTML, no async work between user gesture and window.open).
  // Attachment data_urls are pre-fetched into attDataCache by the useEffect above.
