...
export default function Reports() {
  ...
  const canExport = perms['commercial.export_reports'] || store.currentUser?.role === 'Admin';
            ...
            return (
              <div key={report.id} className="bg-[#1a2236] rounded-xl border border-[#1e2d4a] p-4 hover:border-[#2a3d5a] transition-all group cursor-pointer">
                ...
                {canExport ? (
                  ...
                  <div className="w-full flex items-center justify-center gap-2 py-2 bg-[#0d1628] text-slate-600 rounded-lg text-xs font-semibold border border-[#1e2d4a] cursor-not-allowed select-none">
                    <Download size={12} />Export Restricted
                  </div>
                )}
              </div>
            );
