...
function YNBtn({ value, opt, onSet }: { value: string; opt: string; onSet: (v: string) => void }) {
  ...
  const cls = active
    ...
    : 'bg-transparent border-[#1e2d4a] text-slate-600 hover:border-slate-500 hover:text-slate-400';
  return (
    <button type="button" onClick={() => onSet(opt)}
      className={`flex-1 py-1 rounded-lg text-[10px] font-bold border transition-all ${cls}`}>
      {opt}
    </button>
  );
}
        ...
        <div key={i} className="bg-[#0d1628] border border-[#1e2d4a] rounded-xl p-3.5 space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <div><label className={labelCls}>Date</label>
              <input type="date" value={r.date} onChange={e => upd(i, 'date', e.target.value)} className={inputCls} /></div>
            <div><label className={labelCls}>Area / Room</label>
              <input value={r.areaRoom} onChange={e => upd(i, 'areaRoom', e.target.value)} className={inputCls} placeholder="e.g. Level 3, Flat 14" /></div>
            <div><label className={labelCls}>Outlet / Asset</label>
              <input value={r.outletAsset} onChange={e => upd(i, 'outletAsset', e.target.value)} className={inputCls} placeholder="e.g. HW tap, T-01" /></div>
            <div><label className={labelCls}>System</label>
              <input value={r.system} onChange={e => upd(i, 'system', e.target.value)} className={inputCls} placeholder="e.g. DHWS, LTHW" /></div>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            <div><label className={labelCls}>Duration (mins)</label>
              <input value={r.durationMins} onChange={e => upd(i, 'durationMins', e.target.value)} className={inputCls} placeholder="e.g. 5" /></div>
            <div><label className={labelCls}>Engineer</label>
              <input value={r.engineer} onChange={e => upd(i, 'engineer', e.target.value)} className={inputCls} placeholder="Name" /></div>
            <div><label className={labelCls}>Notes</label>
              <input value={r.rowNotes} onChange={e => upd(i, 'rowNotes', e.target.value)} className={inputCls} placeholder="Optional" /></div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className={`${labelCls} text-[9px]`}>Valve Returned Safe</label>
              <div className="flex gap-1 mt-1.5">
                {['Yes','No','N/A'].map(o => <YNBtn key={o} value={r.valveSafe} opt={o} onSet={v => upd(i, 'valveSafe', v)} />)}
              </div>
            </div>
            <div>
              <label className={`${labelCls} text-[9px]`}>Outlet Capped</label>
              <div className="flex gap-1 mt-1.5">
                {['Yes','No','N/A'].map(o => <YNBtn key={o} value={r.capped} opt={o} onSet={v => upd(i, 'capped', v)} />)}
              </div>
            </div>
            <div>
              <label className={`${labelCls} text-[9px]`}>Water Running Clear</label>
              <div className="flex gap-1 mt-1.5">
                {['Yes','No','N/A'].map(o => <YNBtn key={o} value={r.runningClear} opt={o} onSet={v => upd(i, 'runningClear', v)} />)}
              </div>
            </div>
          </div>
        </div>
