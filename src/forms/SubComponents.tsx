...
export interface FlushingRegisterRow {
  ...
  outletAsset: string;
  system: string;
  flushDurationMins: string;
  valveReturnedSafe: string;
  outletCapped: string;
  waterRunningClear: string;
  engineerName: string;
  rowNotes: string;
}
...
const YN_OPTIONS = ['Y', 'N', 'N/A'] as const;

function YNButtons({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
      ...
      {YN_OPTIONS.map(opt => (
        <button
          ...
          className={`flex-1 py-1.5 rounded-lg text-[10px] font-bold border transition-all ${
            value === opt
              ? opt === 'Y' ? 'bg-emerald-600 border-emerald-600 text-white'
                : opt === 'N' ? 'bg-red-600 border-red-600 text-white'
                : 'bg-slate-600 border-slate-600 text-white'
              : 'bg-transparent border-[#1e2d4a] text-slate-600 hover:border-slate-500 hover:text-slate-400'
          }`}
        >
...
export function FlushingRegisterRows({
  ...
  rows: FlushingRegisterRow[];
  onChange: (rows: FlushingRegisterRow[]) => void;
}) {
  const update = (i: number, field: keyof FlushingRegisterRow, val: string) =>
    onChange(rows.map((r, idx) => idx === i ? { ...r, [field]: val } : r));
  const add = () => onChange([...rows, { ...DEFAULT_FLUSHING_ROW }]);
  const remove = (i: number) => onChange(rows.filter((_, idx) => idx !== i));
  ...
  return (
    <div className="space-y-3">
      {rows.map((r, i) => (
        <div key={i} className="bg-[#0d1628] border border-[#1e2d4a] rounded-xl p-3.5 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-cyan-400">Entry {i + 1}</span>
            <button type="button" onClick={() => remove(i)} className="text-slate-700 hover:text-red-400 transition-colors">
              <X size={13} />
            </button>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            <div>
              ...
              <input value={r.areaRoom} onChange={e => update(i, 'areaRoom', e.target.value)} className={inputCls} placeholder="e.g. Level 3, Flat 14" />
            </div>
            <div>
              <label className={labelCls}>Outlet / Asset</label>
              <input value={r.outletAsset} onChange={e => update(i, 'outletAsset', e.target.value)} className={inputCls} placeholder="e.g. HW tap, T-01" />
            </div>
            <div>
              <label className={labelCls}>System</label>
              <input value={r.system} onChange={e => update(i, 'system', e.target.value)} className={inputCls} placeholder="e.g. DHWS, LTHW" />
            </div>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <div>
              <label className={labelCls}>Flush Duration (mins)</label>
              <input value={r.flushDurationMins} onChange={e => update(i, 'flushDurationMins', e.target.value)} className={inputCls} placeholder="e.g. 5" />
            </div>
            <div>
              <label className={labelCls}>Engineer</label>
              <input value={r.engineerName} onChange={e => update(i, 'engineerName', e.target.value)} className={inputCls} placeholder="Name" />
            </div>
            <div>
              <label className={labelCls}>Notes</label>
              <input value={r.rowNotes} onChange={e => update(i, 'rowNotes', e.target.value)} className={inputCls} placeholder="Optional" />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-3 pt-0.5">
            <div>
              <label className={`${labelCls} text-[9px]`}>Valve Returned Safe</label>
              <YNButtons value={r.valveReturnedSafe} onChange={v => update(i, 'valveReturnedSafe', v)} />
            </div>
            <div>
              <label className={`${labelCls} text-[9px]`}>Outlet Capped</label>
              <YNButtons value={r.outletCapped} onChange={v => update(i, 'outletCapped', v)} />
            </div>
            <div>
              <label className={`${labelCls} text-[9px]`}>Water Running Clear</label>
              <YNButtons value={r.waterRunningClear} onChange={v => update(i, 'waterRunningClear', v)} />
            </div>
          </div>
        </div>
      ))}
      <button
        type="button"
        onClick={add}
        className="w-full py-2.5 border border-dashed border-cyan-900/50 rounded-xl text-xs text-cyan-700 hover:text-cyan-400 hover:border-cyan-700/60 transition-colors font-semibold"
      >
        + Add Flushing Entry
      </button>
    </div>
